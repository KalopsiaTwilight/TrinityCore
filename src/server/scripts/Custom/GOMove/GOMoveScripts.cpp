        ...
        if (ID < SPAWN) // no args
        {
            if (ID >= DELET && ID <= GOTO) // has target (needs retrieve gameobject)
            {
                ...
                else
                {
                    ...
                    PhaseShift p = target->GetPhaseShift();
                    switch (ID)
                    {
                    ...
                    case GROUND:
                    {
                        float ground = target->GetMap()->GetHeight(p, x, y, MAX_HEIGHT);
                        if (ground != INVALID_HEIGHT)
                            GOMove::MoveGameObject(player, x, y, ground, o, p, lowguid);
                    } break;
                    case FLOOR:
                    {
                        float floor = target->GetMap()->GetHeight(p, x, y, z);
        ...
        else if (ARG && ID >= SPAWN)
        {
            if (ID >= NORTH && ID <= PHASE)
            {
                ...
                else
                {
                    ...
                    case PHASE: GOMove::MoveGameObject(player, x, y, z, o, player->GetPhaseShift(), lowguid);                           break;
                    }
                }
            }
            else
            {
                ...
                case SPAWN:
                {
                    if (GOMove::SpawnGameObject(player, player->GetPositionX(), player->GetPositionY(), player->GetPositionZ(), player->GetOrientation(), player->GetPhaseShift(), ARG))
    ...
    class GOMove_spellscript : public SpellScript
    {
        ...
        void HandleAfterCast()
        {
            ...
            if (uint32 entry = GOMove::Store.SpawnQueGet(player->GetGUID()))
                GOMove::SpawnGameObject(player, summonPos->GetPositionX(), summonPos->GetPositionY(), summonPos->GetPositionZ(), player->GetOrientation(), player->GetPhaseShift(), entry);
        }
