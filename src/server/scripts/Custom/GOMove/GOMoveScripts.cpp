        ...
        else if (ARG && ID >= SPAWN)
        {
            if (ID >= NORTH && ID <= PHASE)
            {
                ...
                else
                {
                    ...
                    case ROLL_POS: GOMove::RotateGameObject(player, deg + ((float)ARG), a + 1, lowguid);                          break;
                    case PITCH_POS: GOMove::RotateGameObject(player, deg + ((float)ARG), a + 2, lowguid);                         break;
                    case YAW_POS: GOMove::RotateGameObject(player, deg + ((float)ARG), a + 3, lowguid);                           break;
                    case ROLL_NEG: GOMove::RotateGameObject(player, deg - ((float)ARG), a + 1, lowguid);                          break;
                    case PITCH_NEG: GOMove::RotateGameObject(player, deg - ((float)ARG), a + 2, lowguid);                         break;
                    case YAW_NEG: GOMove::RotateGameObject(player, deg - ((float)ARG), a + 3, lowguid);                           break;
                    }
                }
            }
            else
