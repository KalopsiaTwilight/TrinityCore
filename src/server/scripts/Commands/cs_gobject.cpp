...
public:
    ...
    static bool HandleGameObjectAddCommand(ChatHandler* handler, char const* args)
    {
        ...
        if (objectInfo->displayId && !sGameObjectDisplayInfoStore.LookupEntry(objectInfo->displayId))
        {
            ...
            //TC_LOG_ERROR("sql.sql", "Gameobject (Entry %u GoType: %u) have invalid displayId (%u), not spawned.", objectId, objectInfo->type, objectInfo->displayId); -- CUSTOM, seems some of these are actually fine.  Testing!
            //handler->PSendSysMessage(LANG_GAMEOBJECT_HAVE_INVALID_DATA, objectId);
            TC_LOG_ERROR("sql.sql", "Gameobject (Entry %u GoType: %u) have invalid displayId (%u), but attempted spawn.", objectId, objectInfo->type, objectInfo->displayId);
    ...
    static bool HandleGameObjectSetPhaseCommand(ChatHandler* handler, char const* args)
    {
        ...
        uint32 phaseId = phase ? atoi(phase) : 0;
        if (phaseId == 0)
        {
        ...
        object->SetInPhase(phaseId, true, !object->IsInPhase(phaseId));
        object->SaveToDB();
