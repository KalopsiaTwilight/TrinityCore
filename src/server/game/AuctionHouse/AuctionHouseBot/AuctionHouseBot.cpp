...
void AuctionHouseBot::addNewAuctions(Player *AHBplayer, AHBConfig *config)
{
    ...
    if (maxItems == 0)
    {
        //if (debug_Out) TC_LOG_DEBUG("misc", "AHSeller: Auctions disabled");
        return;
    }
    ...
    {
        //if (debug_Out) TC_LOG_INFO("misc", "AHSeller: Auctions above minimum");
        return;
    }

    if (auctions >= maxItems)
    {
        //if (debug_Out) TC_LOG_INFO("misc", "AHSeller: Auctions at or above maximum");
        return;
    }
...
void AuctionHouseBot::addNewAuctionBuyerBotBid(Player *AHBplayer, AHBConfig *config, WorldSession *session)
{
    if (!AHBBuyer)
    {
        if (debug_Out) TC_LOG_DEBUG("misc", "AHBuyer: Disabled");
        return;
    }
    ...
    for (uint32 count = 1; count <= config->GetBidsPerInterval(); ++count)
    {
        ...
        {
            //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: I have no items to bid on.");
        ...
        if (debug_Out)
        {
            ...
            TC_LOG_DEBUG("misc", "AHBuyer: Ammo Type: %u", prototype->AmmoType);
            TC_LOG_DEBUG("misc", "-------------------------------------------------");
        }
...
void AuctionHouseBot::Update()
{
    ...
    if (!sWorld->getBoolConfig(CONFIG_ALLOW_TWO_SIDE_INTERACTION_AUCTION))
    {
        ...
        {
            //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_a));
            //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: Bidding on Alliance Auctions");
        ...
        {
            //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_h));
            //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: Bidding on Horde Auctions");
    ...
    {
        //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: %u seconds have passed since last bid", (_newrun - _lastrun_n));
        //if (debug_Out) TC_LOG_INFO("misc", "AHBuyer: Bidding on Neutral Auctions");
...
void AuctionHouseBot::Initialize()
{
    ...
    if ((AHBplayerAccount != 0) || (AHBplayerGUID != 0))
    {
        ...
        if (!result)
       {
           TC_LOG_ERROR("misc", "AuctionHouseBot: The account/GUID-information set for your AHBot is incorrect (account: %u guid: %u)", AHBplayerAccount, AHBplayerGUID);
           return;
        }
    }
    ...
    if (AHBSeller)
    {
        ...
        for (ItemTemplateContainer::const_iterator itr = its->begin(); itr != its->end(); ++itr)
        {
           ...
           // Disable Trade Goods above level X
            ...
            // {
            //    if (debug_Out_Filters) TC_LOG_DEBUG("misc", "AuctionHouseBot: Item %u disabled (RequiredSkillRank = %u)", itr->second.ItemId, itr->second.RequiredSkillRank);
            //    continue;
            // }
        ...
        {
            TC_LOG_INFO("misc", "AuctionHouseBot: No items");
            AHBSeller = 0;
        }

        TC_LOG_INFO("misc", "AuctionHouseBot:");
        TC_LOG_INFO("misc", "loaded %u grey trade goods", uint32(greyTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u white trade goods", uint32(whiteTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u green trade goods", uint32(greenTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u blue trade goods", uint32(blueTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u purple trade goods", uint32(purpleTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u orange trade goods", uint32(orangeTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u yellow trade goods", uint32(yellowTradeGoodsBin.size()));
        TC_LOG_INFO("misc", "loaded %u grey items", uint32(greyItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u white items", uint32(whiteItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u green items", uint32(greenItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u blue items", uint32(blueItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u purple items", uint32(purpleItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u orange items", uint32(orangeItemsBin.size()));
        TC_LOG_INFO("misc", "loaded %u yellow items", uint32(yellowItemsBin.size()));
    }
    TC_LOG_INFO("misc", "AuctionHouseBot and AuctionHouseBuyer have been loaded.");
}
...
void AuctionHouseBot::IncrementItemCounts(AuctionEntry* ah)
{
    ...
    if (!u_entry)
    {
        if (debug_Out) TC_LOG_INFO("misc", "AHBot: %u returned as House Faction. Neutral", ah->GetHouseFaction());
        config = &NeutralConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_ALLIANCE)
    {
        if (debug_Out) TC_LOG_INFO("misc", "AHBot: %u returned as House Faction. Alliance", ah->GetHouseFaction());
        config = &AllianceConfig;
    }
    else if (u_entry->ourMask & FACTION_MASK_HORDE)
    {
        if (debug_Out) TC_LOG_INFO("misc", "AHBot: %u returned as House Faction. Horde", ah->GetHouseFaction());
        config = &HordeConfig;
    }
    else
    {
...
{
    if (debug_Out)
        TC_LOG_INFO("misc", "Start Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u", config->GetAHID())->Fetch()->GetCString());
    if (AHBSeller)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_INFO("misc", "minItems                = %u", config->GetMinItems());
            TC_LOG_INFO("misc", "maxItems                = %u", config->GetMaxItems());
            TC_LOG_INFO("misc", "percentGreyTradeGoods   = %u", config->GetPercentages(AHB_GREY_TG));
            TC_LOG_INFO("misc", "percentWhiteTradeGoods  = %u", config->GetPercentages(AHB_WHITE_TG));
            TC_LOG_INFO("misc", "percentGreenTradeGoods  = %u", config->GetPercentages(AHB_GREEN_TG));
            TC_LOG_INFO("misc", "percentBlueTradeGoods   = %u", config->GetPercentages(AHB_BLUE_TG));
            TC_LOG_INFO("misc", "percentPurpleTradeGoods = %u", config->GetPercentages(AHB_PURPLE_TG));
            TC_LOG_INFO("misc", "percentOrangeTradeGoods = %u", config->GetPercentages(AHB_ORANGE_TG));
            TC_LOG_INFO("misc", "percentYellowTradeGoods = %u", config->GetPercentages(AHB_YELLOW_TG));
            TC_LOG_INFO("misc", "percentGreyItems        = %u", config->GetPercentages(AHB_GREY_I));
            TC_LOG_INFO("misc", "percentWhiteItems       = %u", config->GetPercentages(AHB_WHITE_I));
            TC_LOG_INFO("misc", "percentGreenItems       = %u", config->GetPercentages(AHB_GREEN_I));
            TC_LOG_INFO("misc", "percentBlueItems        = %u", config->GetPercentages(AHB_BLUE_I));
            TC_LOG_INFO("misc", "percentPurpleItems      = %u", config->GetPercentages(AHB_PURPLE_I));
            TC_LOG_INFO("misc", "percentOrangeItems      = %u", config->GetPercentages(AHB_ORANGE_I));
            TC_LOG_INFO("misc", "percentYellowItems      = %u", config->GetPercentages(AHB_YELLOW_I));
            TC_LOG_INFO("misc", "minPriceGrey            = %u", config->GetMinPrice(AHB_GREY));
            TC_LOG_INFO("misc", "maxPriceGrey            = %u", config->GetMaxPrice(AHB_GREY));
            TC_LOG_INFO("misc", "minPriceWhite           = %u", config->GetMinPrice(AHB_WHITE));
            TC_LOG_INFO("misc", "maxPriceWhite           = %u", config->GetMaxPrice(AHB_WHITE));
            TC_LOG_INFO("misc", "minPriceGreen           = %u", config->GetMinPrice(AHB_GREEN));
            TC_LOG_INFO("misc", "maxPriceGreen           = %u", config->GetMaxPrice(AHB_GREEN));
            TC_LOG_INFO("misc", "minPriceBlue            = %u", config->GetMinPrice(AHB_BLUE));
            TC_LOG_INFO("misc", "maxPriceBlue            = %u", config->GetMaxPrice(AHB_BLUE));
            TC_LOG_INFO("misc", "minPricePurple          = %u", config->GetMinPrice(AHB_PURPLE));
            TC_LOG_INFO("misc", "maxPricePurple          = %u", config->GetMaxPrice(AHB_PURPLE));
            TC_LOG_INFO("misc", "minPriceOrange          = %u", config->GetMinPrice(AHB_ORANGE));
            TC_LOG_INFO("misc", "maxPriceOrange          = %u", config->GetMaxPrice(AHB_ORANGE));
            TC_LOG_INFO("misc", "minPriceYellow          = %u", config->GetMinPrice(AHB_YELLOW));
            TC_LOG_INFO("misc", "maxPriceYellow          = %u", config->GetMaxPrice(AHB_YELLOW));
            TC_LOG_INFO("misc", "minBidPriceGrey         = %u", config->GetMinBidPrice(AHB_GREY));
            TC_LOG_INFO("misc", "maxBidPriceGrey         = %u", config->GetMaxBidPrice(AHB_GREY));
            TC_LOG_INFO("misc", "minBidPriceWhite        = %u", config->GetMinBidPrice(AHB_WHITE));
            TC_LOG_INFO("misc", "maxBidPriceWhite        = %u", config->GetMaxBidPrice(AHB_WHITE));
            TC_LOG_INFO("misc", "minBidPriceGreen        = %u", config->GetMinBidPrice(AHB_GREEN));
            TC_LOG_INFO("misc", "maxBidPriceGreen        = %u", config->GetMaxBidPrice(AHB_GREEN));
            TC_LOG_INFO("misc", "minBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_INFO("misc", "maxBidPriceBlue         = %u", config->GetMinBidPrice(AHB_BLUE));
            TC_LOG_INFO("misc", "minBidPricePurple       = %u", config->GetMinBidPrice(AHB_PURPLE));
            TC_LOG_INFO("misc", "maxBidPricePurple       = %u", config->GetMaxBidPrice(AHB_PURPLE));
            TC_LOG_INFO("misc", "minBidPriceOrange       = %u", config->GetMinBidPrice(AHB_ORANGE));
            TC_LOG_INFO("misc", "maxBidPriceOrange       = %u", config->GetMaxBidPrice(AHB_ORANGE));
            TC_LOG_INFO("misc", "minBidPriceYellow       = %u", config->GetMinBidPrice(AHB_YELLOW));
            TC_LOG_INFO("misc", "maxBidPriceYellow       = %u", config->GetMaxBidPrice(AHB_YELLOW));
            TC_LOG_INFO("misc", "maxStackGrey            = %u", config->GetMaxStack(AHB_GREY));
            TC_LOG_INFO("misc", "maxStackWhite           = %u", config->GetMaxStack(AHB_WHITE));
            TC_LOG_INFO("misc", "maxStackGreen           = %u", config->GetMaxStack(AHB_GREEN));
            TC_LOG_INFO("misc", "maxStackBlue            = %u", config->GetMaxStack(AHB_BLUE));
            TC_LOG_INFO("misc", "maxStackPurple          = %u", config->GetMaxStack(AHB_PURPLE));
            TC_LOG_INFO("misc", "maxStackOrange          = %u", config->GetMaxStack(AHB_ORANGE));
            TC_LOG_INFO("misc", "maxStackYellow          = %u", config->GetMaxStack(AHB_YELLOW));
        }
        //AuctionHouseEntry const* ahEntry =  sAuctionMgr->GetAuctionHouseEntry(config->GetAHFID());
        AuctionHouseObject* auctionHouse =  sAuctionMgr->GetAuctionsMap(config->GetAHFID());
        ...
        if (debug_Out)
        {
            TC_LOG_INFO("misc", "Current Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetCString());
            TC_LOG_INFO("misc", "Grey Trade Goods\t%u\tGrey Items\t%u", config->GetItemCounts(AHB_GREY_TG), config->GetItemCounts(AHB_GREY_I));
            TC_LOG_INFO("misc", "White Trade Goods\t%u\tWhite Items\t%u", config->GetItemCounts(AHB_WHITE_TG), config->GetItemCounts(AHB_WHITE_I));
            TC_LOG_INFO("misc", "Green Trade Goods\t%u\tGreen Items\t%u", config->GetItemCounts(AHB_GREEN_TG), config->GetItemCounts(AHB_GREEN_I));
            TC_LOG_INFO("misc", "Blue Trade Goods\t%u\tBlue Items\t%u", config->GetItemCounts(AHB_BLUE_TG), config->GetItemCounts(AHB_BLUE_I));
            TC_LOG_INFO("misc", "Purple Trade Goods\t%u\tPurple Items\t%u", config->GetItemCounts(AHB_PURPLE_TG), config->GetItemCounts(AHB_PURPLE_I));
            TC_LOG_INFO("misc", "Orange Trade Goods\t%u\tOrange Items\t%u", config->GetItemCounts(AHB_ORANGE_TG), config->GetItemCounts(AHB_ORANGE_I));
            TC_LOG_INFO("misc", "Yellow Trade Goods\t%u\tYellow Items\t%u", config->GetItemCounts(AHB_YELLOW_TG), config->GetItemCounts(AHB_YELLOW_I));
        }
    }
    if (AHBBuyer)
    {
        ...
        if (debug_Out)
        {
            TC_LOG_INFO("misc", "buyerPriceGrey          = %u", config->GetBuyerPrice(AHB_GREY));
            TC_LOG_INFO("misc", "buyerPriceWhite         = %u", config->GetBuyerPrice(AHB_WHITE));
            TC_LOG_INFO("misc", "buyerPriceGreen         = %u", config->GetBuyerPrice(AHB_GREEN));
            TC_LOG_INFO("misc", "buyerPriceBlue          = %u", config->GetBuyerPrice(AHB_BLUE));
            TC_LOG_INFO("misc", "buyerPricePurple        = %u", config->GetBuyerPrice(AHB_PURPLE));
            TC_LOG_INFO("misc", "buyerPriceOrange        = %u", config->GetBuyerPrice(AHB_ORANGE));
            TC_LOG_INFO("misc", "buyerPriceYellow        = %u", config->GetBuyerPrice(AHB_YELLOW));
            TC_LOG_INFO("misc", "buyerBiddingInterval    = %u", config->GetBiddingInterval());
            TC_LOG_INFO("misc", "buyerBidsPerInterval    = %u", config->GetBidsPerInterval());
        }
    }
    if (debug_Out) TC_LOG_INFO("misc", "End Settings for %s Auctionhouses:", CharacterDatabase.PQuery("SELECT name FROM auctionhousebot WHERE auctionhouse = %u",config->GetAHID())->Fetch()->GetCString());
}