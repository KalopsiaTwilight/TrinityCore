...
void IRCCmd::Handle_Login(_CDATA *CD)
{
    ...
    if (isbanned == "NOTBANNED")
    {
        if (!IsLoggedIn(CD->USER))
        {
            ...
            {
                QueryResult result = LoginDatabase.PQuery("SELECT `username` FROM `account` WHERE `username`='%s' AND `sha_pass_hash`=SHA1(CONCAT(UPPER(`username`),':',UPPER('%s')));", _PARAMS[0].c_str(), _PARAMS[1].c_str());
                if (result)
                {
                    QueryResult result = LoginDatabase.PQuery("SELECT `gmlevel` FROM `account`, `account_access` WHERE `username`='%s' AND `account_access`.`id`=`account`.`id`;", _PARAMS[0].c_str());
                    if (result)
                    {
                        ...
                        if (GMLevel >= 0)
                        {
                            ...
                            {
                                for (int i=1;i < sIRC._chan_count + 1;i++)
                                sIRC.SendIRC("MODE #"+sIRC._irc_chan[i]+" +v "+CD->USER);
                            }
                        }
                    }
                    else
...
void IRCCmd::Inchan_Server(_CDATA *CD)
{
    ...
    QueryResult result = CharacterDatabase.PQuery("SELECT * FROM `irc_inchan` WHERE `channel` = '%s' ORDER BY `name`", _PARAMS[0].c_str());
    if (result)
    {
        Field *fields = result->Fetch();
...
void IRCCmd::Item_Player(_CDATA *CD)
{
    ...
    if (_PARAMS[1] == "add")
    {
        ...
        if (count == 0 || dest.empty())                          // other error, can't add
        {
        ...
        if (Player *plr = GetPlayer(_PARAMS[0]))
        {
            ...
            if (_PARAMS[1] == "release")
            {
                ...
                if (result)
                {
                    ...
                    plr->m_movementInfo.RemoveMovementFlag(UNIT_STATE_ROOT);
            ...
            else
            {
                ...
                plr->TeleportTo(0, 16236.373, 16411.385, -64.38, 3.96);
                plr->m_movementInfo.AddMovementFlag(UNIT_STATE_ROOT);
...
void IRCCmd::Player_Player(_CDATA *CD)
{
    ...
    if (plguid > 0)
    {
        QueryResult result = CharacterDatabase.PQuery("SELECT guid, account, name, race, class, online, SUBSTRING_INDEX(SUBSTRING_INDEX(`level`, ' ' , 35), ' ' , -1) AS level,  SUBSTRING_INDEX(SUBSTRING_INDEX(`xp`, ' ' , 927), ' ' , -1) AS xp, SUBSTRING_INDEX(SUBSTRING_INDEX(money, ' ' , 1462), ' ' , -1) AS gold, totaltime FROM characters WHERE guid =%i", plguid);
        uint32 latency = 0;
        ...
        if (result)
        {
            ...
            std::string totaltim = SecToDay(fields[9].GetString());
            ...
            if (ponline == "\x3\x30\x33Online")
            {
                ...
                if (plr)
                {
                    ...
                    ponline += " in " + (std::string) area->area_name;
                    if (area->zone != 0)
                    {
                        AreaTableEntry const* zone = GetAreaEntryByAreaID(area->zone);
                        ponline += " (" + (std::string)zone->area_name + ")";
                    }
                }
            }
            ...
            std::string pinfo2 = "\00310Race: \xF"+(std::string)prace->name+" |\00310 Class: \xF"+(std::string)pclass->name+" |\00310 Level: \xF"+plevel+" |\00310 Money: \xF"+tempgold+"|\00310 Status: \xF"+ponline;
    ...
    if (_PARAMS[0] == "char")
    {
        ...
        if (plguid > 0)
        {
            ...
            if (result)
            {
                ...
                std::string pinfo2 = "\x2 Race:\x2\x3\x31\x30 " + (std::string)prace->name + "\xF |\x2 Class:\x2\x3\x31\x30 " + (std::string)pclass->name + "\xF |\x2 Level:\x2\x3\x31\x30 " + plevel + "\xF |\x2 Money:\x2 " + tempgold + "\xF |\x2 Status:\x2 " + ponline;
                //        pinfo3 = " :" + " \x2Honor Kills:\x2\x3\x31\x30 " + hk;
                Send_IRCA(ChanOrPM(CD),pinfo , true, CD->TYPE);
                Send_IRCA(ChanOrPM(CD),pinfo2 , true, CD->TYPE);
                //        Send_IRCA(ChanOrPM(CD),pinfo3 , true, CD->TYPE);
...
void IRCCmd::Level_Player(_CDATA *CD)
{
    ...
    {
        Send_IRCA(CD->USER, MakeMsg("Level Must Be Between 1 And %i!",sConfigMgr->GetIntDefault("MaxPlayerLevel", 85)), true, "ERROR");
        return;
    } else
    {
        ...
        if (chr)
        {
            ...
            *data << (uint8)CHAT_MSG_SYSTEM;
            if (i_oldlvl == i_newlvl)
                *data << "Your level progress has been reset.";
            else
            if (i_oldlvl < i_newlvl)
            {
