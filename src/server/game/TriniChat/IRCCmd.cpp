        ...
        else if (CDATA.CMD == "RESTART")
        {
            switch(ParamsValid(&CDATA, 0, sIRC.CSHUTDOWN))
            {
                case E_OK:
                    ...
                    break;
                case E_SIZE:
                    sIRC.Send_IRC_Channel(USER, "\0034[ERROR] : Syntax Error! ("+sIRC._cmd_prefx+"server <setmotd/sec_number_for_restart>)", true, "ERROR");
                    break;
                case E_AUTH:
                    AuthValid = false;
                    break;
        ...
        else if (CDATA.CMD == "SERVER")
        {
            ...
            switch(ParamsValid(&CDATA, 2, sIRC.CSERVERCMD))
            {
                case E_OK:
                    ...
                    break;
                case E_SIZE:
                    sIRC.Send_IRC_Channel(USER, "\0034[ERROR] : Syntax Error! ("+sIRC._cmd_prefx+"server <setmotd>)", true, "ERROR");
                    break;
                case E_AUTH:
                    AuthValid = false;
                    break;
