...
class ObjectGuid
{
    public:
        ...
        ObjectGuid(HighGuid hi, uint32 entry, uint32 counter) { _data._guid = (hi > 70000 || counter) ? uint64(counter) | (uint64(entry) << 32) | (uint64(hi) << ((hi == HIGHGUID_CORPSE || hi == HIGHGUID_AREATRIGGER) ? 48 : 52)) : 0; }
