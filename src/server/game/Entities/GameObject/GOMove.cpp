...
GameObject * GOMove::SpawnGameObject(Player* player, float x, float y, float z, float o, PhaseShift p, uint32 entry)
{
    if (!player || !entry)
        return nullptr;
...
GameObject * GOMove::MoveGameObject(Player* player, float x, float y, float z, float o, PhaseShift p, ObjectGuid::LowType lowguid)
{
    if (!player)
        return nullptr;
    ...
    PhasingHandler::ResetPhaseShift(object);
    // copy paste from WorldObject::CopyPhaseFrom(object, update)
    bool update = true;
    PhasingHandler::InheritPhaseShift(object, player);
    if (update && object->IsInWorld())
